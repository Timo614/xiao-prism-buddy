...
static struct prism_city __g_city_model;
...
static int __city_get(void)
{
    ...
    return __city_data_parse(local_response_buffer);
}
#else
...
static void __prism_http_task(void *p_arg)
{
    int err = -1;
...
int prism_city_init(void)
{
    ...
    xTaskCreate(&__prism_http_task, "__prism_http_task", 1024 * 5, NULL, 10, NULL);
